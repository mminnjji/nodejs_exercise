...
router.post('/login_process', function (request, response) {
    ...
    if (username && password) {             
        ...
        db.query('SELECT * FROM usertable WHERE username = ? AND password = ?', [username, password], function(error, results, fields) {
            ...
            if (results.length > 0) {       
                // 데이터베이스에서 해당 아이디와 비밀번호가 일치하는 사용자가 있는지 확인합니다.
                request.session.is_logined = true;      
                // 세션에 로그인 상태를 저장합니다.
                request.session.nickname = username;
                request.session.save(function () {
                    response.redirect(`/`);
                    // 세션 저장 후 메인 페이지로 리디렉션합니다.
                });
            } else {              
                response.send(`<script type="text/javascript">alert("로그인 정보가 일치하지 않습니다."); 
                document.location.href="/auth/login";</script>`);    
                // 아이디나 비밀번호가 일치하지 않으면 경고 메시지를 띄우고 로그인 페이지로 리디렉션합니다.
            }            
        });
        ...
        document.location.href="/auth/login";</script>`);    
        // 아이디나 비밀번호가 입력되지 않았을 때 경고 메시지를 띄우고 로그인 페이지로 리디렉션합니다.
    }
});
